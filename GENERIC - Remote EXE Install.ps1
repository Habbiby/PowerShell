<# =====================================================================================
    DESCRIPTION
        This script was created to quickly install a specific EXE across multiple Windows hosts
        The installer EXE will always be placed locally in C:\TEMP\ on each specified Windows host

    NOTES
        Install path is not a variable due to limitations of Invoke-Command, so the EXE is executed from C:\TEMP\$File
        Has some inbuilt sleep timers to wait for the EXE file to be copied or installed 
        Seperated the copy loop and the install loop as sometetimes it wouldn't run the EXE for some reason (even with a sleep timer after the copy)
       

    FUNCTIONS
        1. Copies the EXE to the computers local directory
        ...
        2. Installs the EXE from the above directory on each Windows host

        3. Checks registry to see if the EXE was installed successfully 
        ...
        4. Outputs a table with results, including additional information from registry for the installed EXE, if installed successfully

    =====================================================================================
    PREREQUISITES
        1. Access to the respective file server or path to copy the EXE from
...
$Computers = "", "" #List of computers or hosts for the EXE to be installed on
$CopyPath = "\\**********\jre-8u411-x86.exe" #Where the EXE is being copied from (include the file name and .EXE) will be copied to each Windows host
$File = "jre-8u411-x86.exe" #Name of the EXE to be installed, include .EXE
$AppName = "Java" #Input part of the name of the installed app to search for in registry - e.g. check, 7-z, Microsoft, Manager, etc.
#Line 89!!!!!! Couldn't get the variable name passthrough working when it's pointing to a specific file (normally it would be "$Using:File")
...
ForEach ($Comp in $Computers) {
    ...
    IF ($TestPath -eq $True){
        ...
        Invoke-Command -AsJob -ComputerName $comp -Credential $Credentials -scriptblock {start-process "C:\TEMP\jre-8u411-x86.exe" /qn -wait} | out-null       #File name required to be changed here as well
    }
    ELSE{
...
''
Write-host "Waiting 2 minutes for install(s)"
...
$Results = ForEach ($Comp in $Computers) {
    ...
    ELSE{
        Write-Host "$File successfully installed on $Comp"  -ForegroundColor Green 
        $Tableoutput = $Results
    }
}
